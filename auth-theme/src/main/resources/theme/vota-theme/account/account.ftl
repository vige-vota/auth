...
<@layout.mainLayout active='account' bodyClass='user'; section>
    ...
    <form action="${url.accountUrl}" class="form-horizontal" method="post">
        ...
        <div class="form-group ${messagesPerField.printIfExists('account.attributes.vota','has-error')}">
   			<div class="col-sm-2 col-md-2 ${messagesPerField.printIfExists('account.attributes.vota',properties.kcFormGroupErrorClass!)}">
       			<label for="account.attributes.vota" class="control-label">${msg("vota")}</label>
   			</div>
   			<div class="col-sm-10 col-md-10">
       			<input type="text" class="form-control" id="account.attributes.vota" name="account.attributes.vota" value="${(account.attributes.vota!'')}"/>
   			</div>
		</div>
